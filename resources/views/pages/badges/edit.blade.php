...
@section('content')
    <div class="box">
        <div class="box-body">
            <form id="store-badge" action="{{ route('admin.badges.update', [ $badge->id ]) }}" method="post"
                    ...
                    @while(isset($condition))
                        <div class="condition" id="condition-1">
                            ...
                            <div class="col-xs-12">
                                <div class="col-xs-12 no-padding">
                                    ...
                                    <textarea class="form-control" type="text"
                                              value="{{ old('script') }}"
                                              placeholder="Enter script" name="script">{{ $condition->script }}</textarea>
                                </div>
                            </div>
                            <div class="col-xs-4">
                                ...
                                <select class="form-control" id="event_type">
                                    @foreach($eventTypes as $eventType)
                                        <option value="{{ $eventType->id  }}" {{ ($condition->eventType && $condition->eventType->id == $eventType->id) ? 'selected' : ''}}>
                                            {{ $eventType->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-xs-4">
                                <div class="col-xs-12 no-padding ">
                                    <label>Number</label>
                                    <input class="form-control" value="{{ old('number', $condition->number) }}"
